...
export default function CarouselComponent() {
  const projects = [
    ...
    return (
    <>
  <div className='lg:hidden sm:flex flex-col gap-4 w-full items-center mt-4'>
  {projects.map((p) => (
      <Paper
      elevation={3}
      key={1}
      sx={{ backgroundColor: "#6b0afa", borderRadius: "20px", width: "70%", padding: "20px" }}
    >
  ...
  >
    ...
    >
        <div className='flex flex-col items-center justify-center sm:w-60 lg:w-80 h-96'>
          <h1 className='text-2xl font-bold mb-4 text-white'>{p.name}</h1>
          <img
          className='max-w-[50%] max-h-[30%]'
          src={p.urlImage}
